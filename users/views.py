...
class login(APIView):
    def post(self,request):
        ...
        """
        if we uses below way to save cookie for jwt token then we can only save the cokkie in the backend, 
        if we want to use it in frontend/client side we need to use javascript in frontend to save jwt-cookie 
        in the client side.
